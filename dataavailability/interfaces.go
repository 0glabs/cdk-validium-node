...
type StateInterface interface {
	...
	AddCommitment(ctx context.Context, batchNum uint64, blobRequestParams string, blobStatusReply string, dbTx pgx.Tx) error
}
...
type BatchDataProvider interface {
	...
	GetBatchL2Data(batchNum uint64, hash common.Hash, requestParams []BlobRequestParams) ([]byte, error)

	// Get DA backend type
	GetDaBackendType() DABackendType
}
...
type SequenceSender interface {
	...
	PostSequence(ctx context.Context, batchesData [][]byte, batchesNumber []uint64, state StateInterface) ([]byte, error)
}
